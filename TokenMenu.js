...
function display_placed_token_customization_modal(placedToken) {
	...
	const add_token_customization_image = function(imageUrl) {
		...
		if (savedTokenData === undefined) {
			...
			// savedTokenData = find_token_at_path(path, tokenDataName);    // get the most up to date data
			redraw_images_in_modal_body(modalBody, placedToken.options.name, savedTokenData, placedToken);
		}
	};
...
// TODO: handle removing `data-img` urls
