...
class EncounterHandler {
	...
	get combat_iframe() {
		...
		if (window.EncounterHandler.is_loading) {
			...
			iframe = $(".iframe-encounter-combat-tracker-is-loading");
		}
		if (iframe === undefined) {
			// we don't have a local variable. Try to set it to whatever matching iframe we can find
			iframe = $(".iframe-encounter-combat-tracker");
		}
		if (iframe === undefined) {
			// we still don't have an iframe? This should not be possible
			console.warn("EncounterHandler.combat_iframe is undefined!!!");
		}
		return iframe
	}
	...
	get currently_open_token_id() {
		return window.EncounterHandler.combat_iframe.attr("token");
	}
	...
	combat_iframe_did_load() {
		...
		open_monster_stat_block(window.EncounterHandler.currently_open_monster_id, window.EncounterHandler.currently_open_token_id);
...
function close_monster_stat_block() {
	...
	console.log("close_monster_stat_block is closing the stat block")

	$(".iframe-encounter-combat-tracker").hide(); // hide all that we find. Even if we're currently loading one

	window.EncounterHandler.combat_iframe.attr("data-monster", undefined);
	window.EncounterHandler.combat_iframe.attr("data-token", undefined);
...
function open_monster_stat_block(monsterId, tokenId) {
	if (window.EncounterHandler === undefined) {
		// only the DM should have an EncounterHandler. If they don't for some reason, we have a problem.
		if (window.DM) {
			console.warn("open_monster_stat_block was called without an EncounterHandler!!!");
		}
		return;
	}
	...
	if (monsterId === undefined) {
		...
		console.log("open_monster_stat_block was called without a monsterId");
		return;
	}
	...
	if (monster !== undefined) {
		...
		found = click_combat_monster_with_name(monster.name);
	}

	if (!found) {
		if (window.EncounterHandler.is_loading) {
			console.log(`open_monster_stat_block could not find the monster on the page, but we're activiely loading so we'll wait for loading to finish and try again`);
		} else {
			console.log(`open_monster_stat_block could not find the monster on the page. Attempting to update the encounter and try again`);
		...
		console.log("open_monster_stat_block is showing the loading indicator");
		display_combat_tracker_loading_indicator();
		window.EncounterHandler.combat_iframe.show();
	} else {
		console.log("open_monster_stat_block is showing a stat block");
		...
		fetch_and_inject_monster_image(monsterId);
...
function fetch_and_inject_monster_image(monsterId) {
	...
	window.StatHandler.getStat(monsterId, function(stat) {
		if (window.EncounterHandler.currently_open_monster_id == monsterId) {
...
function reposition_enounter_combat_tracker_iframe() {
	...
	console.log("reposition_enounter_combat_tracker_iframe starting");
