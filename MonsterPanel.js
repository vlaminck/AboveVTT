...
function monster_panel_init_loop() {
	...
	let monsterRows = iframeBody.find(".monster-row");
	if (monsterRows.length > 0) {
		...
		if (editLink.length > 0) {
			...
			iframeBody.find(".ddb-page-header__controls .encounter-details__actions > button").click();
	...
	if (!window.EncounterHandler.has_avtt_encounter() || !window.EncounterHandler.encounterBuilderDiceSupported) {
		...
		monsterRow.click(function(event) {
			window.StatHandler.getStat(monsterId, function(stat) {
				...
				}, 1000); // TODO: if this amount of time is a requirement, then we should change this code to be reactive to avoid race conditions
			});
		});
	}
